jQuery(function ($) {
  ...
  function renderQuote(quote) {
    ...
    let quoteText = $(
      '<div id="quote-text" class="h3 py-5 mb-4 w-100 font-weight-light text-secondary border-bottom border-gray"></div>'
    );
  ...
  function renderVoiceControls(synthesis, voice) {
    let controlsPane = $(
      '<div id="voice-controls-pane" class="d-flex flex-wrap w-100 align-items-center align-content-center justify-content-between"></div>'
    );
    ...
    if (voice) {
      let currentVoice = $(
        '<div class="text-secondary font-weight-normal"><span class="text-dark font-weight-bold">' +
          voice.name +
          "</span> (" +
          voice.lang +
          ")</div>"
  ...
  function initialize() {
    if ("speechSynthesis" in window) {
      ...
      let notice = $(
        '<div class="w-100 py-4 bg-danger font-weight-bold text-white position-absolute text-center" style="bottom:0; z-index:10">' +
          message +
          "</div>"
      );
